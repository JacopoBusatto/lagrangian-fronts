...
The terminology follows the oceanographic convention in which volume flux per unit area has dimensions of velocity, while volume transport through a finite section has dimensions of volume/time; 1 Sv = 10⁶ m³/s. Here the estimate comes from row-normalized trajectory transitions between cell centres. It does not establish a volume-weighted ocean current or a measured volume transport: the calculation has no represented water-volume weights or section-area integral.
